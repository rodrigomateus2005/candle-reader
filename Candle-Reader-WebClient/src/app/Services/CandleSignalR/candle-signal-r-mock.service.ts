...
export class CandleSignalRMockService extends CandleSignalRService {
  ...
  constructor() {
    ...
    setInterval(() => {
      if (this.lastPrice) {
        const alteracao = (Math.ceil(Math.random() * 10) - 5) / 100;

        this.lastPrice += alteracao;

        this.onPriceChange(this.lastPrice);
      }
    }, 2000);
  }
  ...
  public getCandles(): Promise<Candle[]> {
    ...
    this.lastPrice = retorno[retorno.length - 1].close;

    return Promise.resolve(retorno);
  }

  private onPriceChange(price: number) {
